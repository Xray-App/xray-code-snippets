...
def appendRows(issueID='', testSummary='', testPriority='', action='', data='', result=''):
...
def handleTestCases(root, issueID, outputfile):
    ...
    for testcase in root:
        ...
        for step in testcase.findall('steps/step'):
            ...
            if first_step:
                appendRows(issueID=issueID,testSummary=summary,testPriority=test_priority,action=action,result=expectedResult)
                first_step = False
            else:
                if action and expectedResult:
                    appendRows(issueID=issueID, action=action,result=expectedResult)
        
        if not hasSteps:
            appendRows(issueID=issueID,testSummary=summary,testPriority=test_priority)
        issueID = issueID+1  
