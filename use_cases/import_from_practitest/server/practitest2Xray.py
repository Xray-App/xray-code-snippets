    ...
    for index, xls_row in df.iterrows():
        ...
        if (testType == 'ScriptedTest' or (lastTestType == 'ScriptedTest' and pd.isna(testType))):
            ...
            if testStep == 1:
                ...
                appendRows(issueID=issueID, testType=testType, testSummary=xls_row['Name'], testPriority='', action=action, result=xls_row['Step expected_results'], data='', description=xls_row['Description'], precondition='', unstrusturedDefinition='', gherkinDefinition=xls_row['Scenario'])
            elif testStep > 1:
                #Steps
...
def main(argv):
   ...
   #inputfile='/Users/cristianocunha/Documents/Projects/tutorials/xray-code-snippets/use_cases/import_from_practitest/server/practitest_export.csv'
   #outputfile='/Users/cristianocunha/Documents/Projects/tutorials/xray-code-snippets/use_cases/import_from_practitest/server/practitest_xray_results.csv'
